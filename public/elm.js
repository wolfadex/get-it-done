// elm-watch hot {"version":"1.0.2","webSocketPort":53619}
"use strict";
(() => {
...
${variant}`;
  ...
  var VERSION = "1.0.2";
  var TARGET_NAME = "Get it done app";
  var INITIAL_ELM_COMPILED_TIMESTAMP = Number(
    "1666675418091"
  );
  var ORIGINAL_COMPILATION_MODE = "standard";
  var WEBSOCKET_PORT = "53619";
...
var $author$project$Main$AuthenticatedMsg = function (a) {
	return {$: 'AuthenticatedMsg', a: a};
};
...
var $author$project$Env$apiKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBpd3ZhaGpmamp1enpyZm5pd2N3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE2NjY2NTg3NDksImV4cCI6MTk4MjIzNDc0OX0.JWqbRoYtHhfsQ0F3i1D-11nkO7kwUgo9EqaCHiq4Ct0';
var $author$project$Env$baseUrl = 'https://piwvahjfjjuzzrfniwcw.supabase.co/';
...
var $author$project$Api$UserId = function (a) {
	return {$: 'UserId', a: a};
};
...
var $author$project$Api$resolveRefresh = F2(
...
var $author$project$Api$decodeSignin = function (expect) {
	return A5(
		$elm$json$Json$Decode$map4,
		F4(
			function (accessToken, expiresIn, refreshToken, userId) {
				return _Utils_Tuple3(
					$author$project$Api$AccessToken(accessToken),
					userId,
					A2(
						...
						A2(
							$elm$core$Task$andThen,
							function (_v0) {
								return A2($author$project$Api$refreshTask, expect, refreshToken);
							},
							$elm$core$Process$sleep((expiresIn - 180) * 1000))));
			}),
...
var $author$project$Api$refreshTask = F2(
	function (expect, refreshToken) {
		return $elm$http$Http$task(
			{
				...
				resolver: $elm$http$Http$stringResolver(
					$author$project$Api$resolveRefresh(
						$author$project$Api$decodeSignin(expect))),
				timeout: $elm$core$Maybe$Nothing,
				url: $author$project$Env$baseUrl + 'auth/v1/token?grant_type=refresh_token'
			});
	});
...
var $author$project$Api$signin = function (options) {
	return $elm$http$Http$request(
		{
			body: $elm$http$Http$jsonBody(
				$elm$json$Json$Encode$object(
					_List_fromArray(
						[
							_Utils_Tuple2(
							'email',
							$elm$json$Json$Encode$string(options.email)),
							_Utils_Tuple2(
							'password',
							$elm$json$Json$Encode$string(options.password))
						]))),
			expect: A2(
				$elm$http$Http$expectJson,
				options.expect,
				$author$project$Api$decodeSignin(options.expect)),
			headers: _List_fromArray(
				[
					A2($elm$http$Http$header, 'apikey', $author$project$Env$apiKey)
				]),
			method: 'POST',
			timeout: $elm$core$Maybe$Nothing,
			tracker: $elm$core$Maybe$Nothing,
			url: $author$project$Env$baseUrl + 'auth/v1/token?grant_type=password'
		});
};
		...
		switch (msg.$) {
			...
			case 'GotNewTaskDescription':
				...
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 'CreateTask':
				var newTask = {completed: false, description: model.newTaskDescription, summary: model.newTaskSummary};
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
					...
					_Debug_todo(
						'Main',
						{
							start: {line: 181, column: 15},
							end: {line: 181, column: 25}
						})(''));
			case 'MarkComplete':
				var _v1 = model.tasks;
				if (!_v1.b) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								recentlyCompleted: A2(
									$elm$core$List$take,
									50,
									A2($elm$core$List$cons, first, model.recentlyCompleted)),
								tasks: rest,
			...
			case 'RequestDelete':
				return _Utils_Tuple2(
					model,
					$author$project$Main$toggleDialog('confirm-delete-modal'));
			case 'CancelDelete':
				return _Utils_Tuple2(
					model,
					$author$project$Main$toggleDialog('confirm-delete-modal'));
			case 'ConfirmDelete':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							tasks: A2($elm$core$List$drop, 1, model.tasks)
						}),
					$author$project$Main$toggleDialog('confirm-delete-modal'));
			default:
				if (msg.a.$ === 'Err') {
			...
			switch (_v0.a.$) {
				case 'AuthenticatedMsg':
					if (_v0.b.$ === 'Authenticated') {
						...
						return A3(
							...
							$elm$core$Platform$Cmd$map($author$project$Main$AuthenticatedMsg),
							A2($author$project$Main$updateAuthenticated, authMsg, authModel));
					} else {
						break _v0$7;
					}
				case 'GotEmail':
					if (_v0.b.$ === 'Unauthenticated') {
						var email = _v0.a.a;
						var unauthModel = _v0.b.a;
						return _Utils_Tuple2(
							$author$project$Main$Unauthenticated(
								_Utils_update(
									unauthModel,
				...
				case 'LoginRequested':
					if (_v0.b.$ === 'Unauthenticated') {
						...
						var unauthModel = _v0.b.a;
						return _Utils_Tuple2(
							$author$project$Main$Unauthenticated(unauthModel),
							$author$project$Api$signin(
								{email: form.email, expect: $author$project$Main$LoginResponse, password: form.password}));
					} else {
						break _v0$7;
					}
				case 'LoginResponse':
					if (_v0.a.a.$ === 'Err') {
						...
						if (_v0.b.$ === 'Authenticated') {
							...
							var userId = _v2.b;
							var refreshCmd = _v2.c;
							return _Utils_Tuple2(
								$author$project$Main$Authenticated(
									{accessToken: accessToken, newTaskDescription: '', newTaskSummary: '', recentlyCompleted: _List_Nil, tasks: _List_Nil, totalCompleted: 0, userId: userId}),
								refreshCmd);
						}
					}
				default:
...
var $author$project$Main$GotNewTaskDescription = function (a) {
	return {$: 'GotNewTaskDescription', a: a};
};
var $author$project$Main$GotNewTaskSummary = function (a) {
	return {$: 'GotNewTaskSummary', a: a};
};
...
var $author$project$Main$MarkComplete = {$: 'MarkComplete'};
var $author$project$Main$RequestDelete = {$: 'RequestDelete'};
...
var $author$project$Main$viewCurrentTask = function (task) {
	return A2(
			...
			[
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$text(task.summary)),
						$mdgriffith$elm_ui$Element$text(task.description)
					])),
				A2(
				$mdgriffith$elm_ui$Element$column,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_Nil,
						{
							label: $mdgriffith$elm_ui$Element$text('Complete'),
							onPress: $elm$core$Maybe$Just($author$project$Main$MarkComplete)
						}),
						A2(
						$mdgriffith$elm_ui$Element$Input$button,
						_List_Nil,
						{
							...
							onPress: $elm$core$Maybe$Just($author$project$Main$RequestDelete)
						})
					]))
			]));
};
...
var $author$project$Main$viewModel = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$text('Get It Done!'),
				A2(
				$mdgriffith$elm_ui$Element$column,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						_List_Nil,
						{
							...
							onChange: $author$project$Main$GotNewTaskSummary,
						...
						_List_Nil,
						{
							...
							onChange: $author$project$Main$GotNewTaskDescription,
							placeholder: $elm$core$Maybe$Nothing,
							spellcheck: true,
						...
						_List_Nil,
						{
							label: $mdgriffith$elm_ui$Element$text('Add task'),
							onPress: $elm$core$String$isEmpty(model.newTaskSummary) ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just($author$project$Main$CreateTask)
						})
					])),
				function () {
				var _v0 = model.tasks;
				if (!_v0.b) {
					return $mdgriffith$elm_ui$Element$text('Do whatever!');
				} else {
					var current = _v0.a;
					var rest = _v0.b;
					return A2(
						...
						_List_fromArray(
							[
								$author$project$Main$viewCurrentTask(current),
								A2(
								$mdgriffith$elm_ui$Element$column,
								_List_fromArray(
									[
...
var $author$project$Main$viewAuth = function (model) {
	return _List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$id('confirm-delete-modal')
				]),
			_List_fromArray(
				[
					A3(
						...
						_List_fromArray(
							[
								...
								_List_Nil,
								{
									...
									onPress: $elm$core$Maybe$Just($author$project$Main$CancelDelete)
								}),
								A2(
								$mdgriffith$elm_ui$Element$Input$button,
								_List_Nil,
...
var $author$project$Main$GotPassword = function (a) {
	return {$: 'GotPassword', a: a};
};
var $author$project$Main$LoginRequested = function (a) {
	return {$: 'LoginRequested', a: a};
};
...
var $author$project$Main$viewUnauth = function (model) {
	return _List_fromArray(
		[
			A2(
			$mdgriffith$elm_ui$Element$layout,
				...
				_List_fromArray(
					[
						...
						_List_Nil,
						{
							...
							onChange: $author$project$Main$GotEmail,
							placeholder: $elm$core$Maybe$Nothing,
							text: model.email
						}),
						A2(
						...
						_List_Nil,
						{
							...
							onChange: $author$project$Main$GotPassword,
							placeholder: $elm$core$Maybe$Nothing,
							show: false,
							text: model.password
						}),
						...
						_List_Nil,
						{
							...
							onPress: ($elm$core$String$isEmpty(model.email) || $elm$core$String$isEmpty(model.password)) ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just(
								$author$project$Main$LoginRequested(model))
						})
					])))
		]);
};
	...
	return {
		body: function () {
			if (model.$ === 'Unauthenticated') {
				var unauthModel = model.a;
				return $author$project$Main$viewUnauth(unauthModel);
			} else {
				var authModel = model.a;
				return A2(
					$elm$core$List$map,
					$elm$html$Html$map($author$project$Main$AuthenticatedMsg),
					$author$project$Main$viewAuth(authModel));
			}
		}(),
		title: 'Get It Done!'
	};
